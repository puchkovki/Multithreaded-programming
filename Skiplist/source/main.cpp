...
#define MEMORYRELAXED                         std::memory_order_relaxed
#define ATM_GET(var)                (var).load(MEMORYRELAXED)
#define ATM_LOAD(var, val)          (val) = (var).load(MEMORYRELAXED)
#define ATM_STORE(var, val)         (var).store((val), MEMORYRELAXED)
#define ATM_CAS(var, exp, val)      (var).compare_exchange_weak((exp), (val))
#define ATM_FETCH_ADD(var, val)     (var).fetch_add(val, MEMORYRELAXED)
#define ATM_FETCH_SUB(var, val)     (var).fetch_sub(val, MEMORYRELAXED)
#define ALLOC_(type, var, count)    (var) = new type[count]
#define FREE_(var)                  delete[] (var)


static inline void _sl_node_init(skiplist_node *node, size_t top_layer)
{
